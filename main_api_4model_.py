...
if uploaded_file:
    ...
    user_prompt += f"Considering the individual named {name} is {age} years old and weighs {weight} kg, what would be your nutritional advice. Act as a personal nutritional specialist and communicate to the user in a human like manner:?\n"

    advice_container = st.container()
    with advice_container:
        st.subheader("Initial Nutritional Advice")
        try:
    ...
    with chat_container:
        st.subheader("Chat with Nutritionist Bot")
        ...
        if user_input:
            try:
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo",
                    messages=[{"role": "system", "content": "You are a helpful assistant."},
                              {"role": "user", "content": f"{user_input} Given the information above, please elaborate more specifically on the dietary advice:"}]
                )
                st.write("Nutritionist Bot:", response.choices[0].text.strip())
            except Exception as e:
